...
class PyMISPGhidraIOHandler:
    ...
    def handle_parameter(self, param, param_type):
        ...
        if self.isHeadless: 
            self.handle_exception_message(ValueError(f"Missing parameter {param_type}"), f"Error handling parameter {param_type}")
        ...
        user_GUI_input_handles = {
            ...
            "function-address": lambda: self.interpreter.currentAddress.toString()  
        }
        ...
        if param_type in user_GUI_input_handles:
            try:
                ...
                self.handle_exception_message(e, f"Error handling user input for parameter {param_type}")

        # If param_type is not in user_GUI_input_handles, raise an error
        self.handle_exception_message(ValueError(f"Unknown parameter type {param_type}"), f"Error handling parameter {param_type}")
    ...
    def handle_message(self, message):
        ...
        if self.isHeadless is False: self.interpreter.popup(message)
